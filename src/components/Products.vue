                ...
                <v-tooltip bottom>
                  <template v-slot:activator="{ on }">
                    <v-btn
                      ...
                      ><v-icon large dark>mdi-cart-plus</v-icon></v-btn
                    >
                  </template>
...
export default {
  ...
  methods: {
    fetchproducts() {
      Vue.axios
        .get("https://eshop-deve.herokuapp.com/api/v2/orders/2117155815564", {
          headers: {
        ...
        .then(response => {
          ...
          this.products = this.data.order.items.map(function(data) {
            return {
              sku: data.sku,
              name: data.name,
    ...
    showpayproduct() {
      this.$swal(
        "Pago recibido!",
        "Se ha realizado sadisfactoriamente el pago!",
        "success"
      );
    }
  }
